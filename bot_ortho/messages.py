...
TOPICS = {
    ...
    15: "-Н- и -НН- в суффиксах",
}

GREETING = (
    "Привет! 👋\n\n"
    "Я тренажёр орфографии для ЕГЭ — задания <b>9–12</b> и <b>15</b>:\n"
    "• №9 — правописание корней\n"
    "• №10 — правописание приставок\n"
    "• №11 — правописание суффиксов\n"
    "• №12 — личные окончания глаголов и суффиксы причастий\n"
    "• №15 — -Н- и -НН- в суффиксах\n\n"
    "В №9–12 показываю один ряд слов с пропусками — ты пишешь буквы, которых "
    "не хватает, через запятую и по порядку слов. Например для ряда "
    "<i>сош..ют, пас..янс, без..языкий</i> ответ: <code>ь, ь, ъ</code>.\n\n"
    "В №15 показываю предложение с пронумерованными пропусками — ты пишешь "
    "цифры, на месте которых нужна двойная буква.\n\n"
...
HELP = (
    ...
    "/stats — статистика\n\n"
    "Ответ в №9–12 — пропущенные буквы по порядку слов: <code>ь, ь, ъ</code>, "
    "<code>ь ь ъ</code> или <code>ььъ</code>. Ё можно писать как Е.\n"
    "Ответ в №15 — цифры: <code>124</code> или <code>1, 2, 4</code>."
)
...
ASK = {
    "digits": "Напиши цифры — можно подряд: <code>124</code>.",
...
NUDGE = {
    "digits": (
        "Жду только цифры — например <code>124</code>. "
...
# The numbered gaps of №15: "художестве(1)ого" — worth making them stand out.
...
def render_task(task: dict, seq: int | None = None) -> str:
    ...
    if kind == "digits":
        body = GAP_MARK_RE.sub(r"<b>(\1)</b>", html.escape(task["sentence"]))
    elif kind == "letters":
