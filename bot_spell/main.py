"""Слитно / раздельно / через дефис trainer (ЕГЭ №13–14) Telegram bot."""
...
logger = logging.getLogger("spell-bot")

ROOT = Path(__file__).resolve().parent
load_dotenv(ROOT / ".env")

BOT_TOKEN = os.environ["BOT_TOKEN"]
SPELL_DATA = (ROOT / os.environ.get("SPELL_DATA", "../out/spell.json")).resolve()
...
def load_tasks() -> dict[str, dict]:
    raw = json.loads(SPELL_DATA.read_text(encoding="utf-8"))
...
def kb_topics(current: int | None) -> InlineKeyboardMarkup:
    ...
    rows = [[InlineKeyboardButton(text=label(None, "Обе вперемешку"), callback_data="topic:all")]]
...
async def on_answer(message: Message, state: FSMContext) -> None:
    ...
    if task_id is None:
        ...
        return
    task = TASKS[task_id]
    if not answer.looks_like_answer(message.text):
        await message.answer(messages.NUDGE)
        return
    correct = answer.is_correct(message.text, task["answer"])
